...
#        print >> sys.stderr, '\n*** File __init__',fs, path,flags,mode
        ...
        self.__db = fs.db               # Database
        self.__ll = fs.ll               # Low level implementation
        ...
        if self.__inode == -errno.ENOENT:
            ...
            self.__inode = self.__db.mknod()
            self.__db.link(parent_dir_inode,name,self.__inode)
...
#        return -errno.ENOSYS
    ...
    def ftruncate(self, length):
        ...
        for chunk in self.__db.Get_Chunks_Truncate(self.__inode, ceil):
            if self.__Split_Chunks(chunk):
                self._Free_Chunks(chunk)

        # Set new file size
        self.__db.Set_Size(inode,length)
...
#        print >> sys.stderr, '*** read', length,offset
        ...
        # Get file size
        size = self.__db.Get_Size(self.__inode)
...
#        return -errno.ENOSYS
    ...
    def write(self, data,offset):                                               # OK
        ...
        if offset:
            ...
            if sector:
                sector = self.__ll.Read([{"sector":sector,"length":1}])
...
#        print >> sys.stderr, "chunks_write",repr(chunks_write)
        for chunk in chunks_write:
            ...
            self.__ll.Write_Chunk(chunk['sector'], d)
...
#        plugins.send("File.write", chunks_write=chunks_write, data=data)
        ...
        if file_size > self.__db.Get_Size(self.__inode):
            self.__db.Set_Size(self.__inode,file_size)
...
#        print >> sys.stderr, '\tGet_Chunks', file,floor,ceil
        ...
        # Stored chunks
        chunks = self.__db.Get_Chunks(self.__inode,floor,ceil)
...
#        print >> sys.stderr, '*** __Get_FreeSpace', sectors_required
        ...
        while sectors_required > 0:
            chunk = self.__db.Get_FreeSpace(sectors_required,chunks)
    ...
    def __Split_Chunks(self, chunk):
        if self.__db.Split_Chunks(chunk):
